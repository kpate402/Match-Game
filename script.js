function createNewCard() {
  ...
  // cardDown = document.createElement("div").classList.add("card-down");
  // cardUp = document.createElement("div").classList.add("card-up");
	

	/* Step 2: Add the "card" class to the variable 'cardElement' from the previous step. */
	// cardElement.ChildNodes.add("div").classList.add("card-down");
 //  cardElement.ChildNodes.add("div").classList.add("card-up");
  ...
  // html = "<div class='card-down'></div> <div class='card-up'></div> ";
...
createNewCardTest();
// console.log(createNewCard().innerHTML);
...
function shuffleCardImageClasses() {
  ...
  let temp = _.shuffle(imageList);
  let cardClasses2 = [];
  for(let i = 0; i < 6; i++) {
    cardClasses2.push(temp[i]);
    cardClasses2.push(temp[i]);
  }

  let cardRandom = _.shuffle(cardClasses2);
  ...
  NOTE: Ignore the "require" syntax shown in the documentation as this is for non-browser environments. The '_' variable will already be available to you from loading the CDN. */
   //let cardRandom = _.shuffle(cardClasses);
...
function onCardFlipped(newlyFlippedCard) {
  ...
  let match = doCardsMatch(lastCardFlipped, newlyFlippedCard)
